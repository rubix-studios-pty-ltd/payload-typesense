...
export function generateThemeClasses(
	...
	const resultsContainerStyles = css({
		...
		borderRadius: enableRoundedCorners
			? theme.spacing.resultsBorderRadius
			: "0",
	...
	const resultItemStyles = css({
		":focus": {
			...
			boxShadow: enableShadows
				? `inset 0 0 0 2px ${theme.colors.inputBorderFocus}`
				: "none",
		},
		":hover": {
	...
	const focusableStyles = css({
		":focus": {
			boxShadow: enableShadows
				? `0 0 0 2px ${theme.colors.inputBorderFocus}`
				: "none",
		},
		outline: "none",
	})
...
export function applyTheme(
	...
	if (variant) {
		...
		return `${(classes as any)[element] || ""} ${
			(classes as any)[variantKey] || ""
		}`
	}
