...
import {
	createDetailedHealthCheckHandler,
...
const searchAllCollections = async (
	...
	try {
		...
		const enabledCollections = Object.entries(
				...
				try {
					const searchParameters: Record<string, unknown> = {
						highlight_full_fields:
							config?.searchFields?.join(",") || "title,content",
						num_typos: 0,
						page: options.page,
...
export const createSearchEndpoints = (
	...
	return [
		...
		{
			handler: createDetailedHealthCheckHandler(
				typesenseClient,
				pluginOptions,
				lastSyncTime
			),
		...
		try {
			...
			if (request.url && typeof request.url === "string") {
				...
				const { collectionName: paramCollectionName } =
					(params as { collectionName?: string }) || {}
			...
			if (!collectionName) {
				...
				return await searchAllCollections(
					typesenseClient,
					pluginOptions,
					q,
					searchOptions
...
const createSuggestHandler = (
	...
	return async (request: any) => {
		...
		if (
			!collectionName ||
			!pluginOptions.collections?.[collectionNameStr]?.enabled
		) {
			return Response.json(
				{ error: "Collection not enabled for search" },
				{ status: 400 }
...
const createCollectionsHandler = (
	pluginOptions: TypesenseSearchConfig
): PayloadHandler => {
	return () => {
		try {
			const collections = Object.entries(pluginOptions.collections || {})
				...
				.map(([slug, config]) => ({
					slug,
					displayName:
						config?.displayName || slug.charAt(0).toUpperCase() + slug.slice(1),
